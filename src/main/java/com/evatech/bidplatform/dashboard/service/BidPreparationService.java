package com.evatech.bidplatform.dashboard.service;

import com.evatech.bidplatform.bid.entity.BidDocumentType;
import com.evatech.bidplatform.dashboard.dto.bid.request.GenerateBidDocumentRequest;
import com.evatech.bidplatform.dashboard.dto.bid.request.SubmitBidForApprovalRequest;
import com.evatech.bidplatform.dashboard.dto.bid.response.*;
import com.evatech.bidplatform.user.entity.User;
import org.springframework.core.io.Resource;
import org.springframework.web.multipart.MultipartFile;

import java.util.List;

public interface BidPreparationService {

    LatestBidDocumentResponse getLatestBidDocument(
            Long contractId,
            User user,
            List<String> roles
    );

    GeneratedBidDocumentResponse generateBidDocument(
            Long contractId,
            GenerateBidDocumentRequest request,
            User user,
            List<String> roles
    );

    BidDocumentDetailsResponse getDocumentDetails(
            Long contractId,
            Long documentId,
            User user,
            List<String> roles
    );

    Resource downloadDocument(
            Long contractId,
            Long documentId,
            User user,
            List<String> roles
    );

    Resource previewDocument(
            Long contractId,
            Long documentId,
            User user,
            List<String> roles
    );

    UploadedBidDocumentResponse uploadCompletedBid(
            Long contractId,
            Long sourceGeneratedDocumentId,
            BidDocumentType documentType,
            MultipartFile file,
            User user,
            List<String> roles
    );

    SubmitBidForApprovalResponse submitForApproval(
            Long contractId,
            SubmitBidForApprovalRequest request,
            User user,
            List<String> roles
    );

    BidPreparationStatusResponse getPreparationStatus(
            Long contractId,
            User user,
            List<String> roles
    );
}
